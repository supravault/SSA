...
function deriveWalletPerformedMetrics(moduleScans: any[]) {
  const performed: any = {
    ...
    l2: {
      // Behavioral/exposure signals must be produced by scanners/agent modules.
      // Here we record concrete evidence if present, otherwise keep metrics null.
  ...
  for (const ms of moduleScans) {
    ...
    // Opaque ABI detection (if scanner stores it)
    const opaque =
    ...
    // Entrypoints count (if scanner stores it)
    const epCount =
    ...
    // Behavioral evidence (if any module scan provides it)
    // NOTE: ideally this comes from a wallet-level behavior module, not per-module.
    const txSampled =
      asNum(ms?.meta?.behavior?.tx_sampled) ?? asNum(ms?.meta?.agent?.behavior?.tx_sampled) ?? null;
    const windowDays =
      asNum(ms?.meta?.behavior?.window_days) ?? asNum(ms?.meta?.agent?.behavior?.window_days) ?? null;
    const uniq =
    ...
    // Attribution/risk model evidence (Level 3)
    ...
    const ver = asStr(ms?.meta?.risk_model?.model_version) ?? asStr(ms?.meta?.agent?.risk_model?.model_version) ?? null;
  ...
  // De-dupe sources/calls/signals while keeping order
...
async function scanWallet(
  ...
  for (const module of modules) {
    ...
    try {
      ...
      moduleScans.push({
        ...
        // ✅ keep per-module classification + reason (if your rules set it)
  ...
  // Proof-of-work metrics (best-effort, derived from actual module scan meta)
  const performed = deriveWalletPerformedMetrics(moduleScans);

  // If Level 2/3 requested but no behavior/attribution evidence exists yet,
  // record concrete "sources used" so summary.ts can phrase it honestly.
  if (level >= 2 && performed.l2.sources_used.length === 0) {
    // These are the actual sources the wallet scan used today.
    performed.l2.sources_used = ["rpc"]; // add "suprascan" later when you implement tx behavior fetch
  }
  if (level >= 3 && performed.l3.sources_used.length === 0) {
    performed.l3.sources_used = ["rpc"]; // add indexer/explorer when attribution is implemented
  }
  ...
  return {
    ...
    meta: {
      ...
      // ✅ NEW: customer-facing proof-of-work payload used by summary.json
      performed,
    } as any,
  };
}
...
program
  ...
  .action(async (options) => {
    try {
      ...
      // Ensure meta.performed exists for coin/fa too (so summary.json can be directed)
      if (!isObj((scanResult as any).meta)) (scanResult as any).meta = {};
      if (!isObj((scanResult as any).meta.performed)) (scanResult as any).meta.performed = {};
      ...
      // Generate summary.json (includes badge + work_performed derived from scanResult.meta.performed)
